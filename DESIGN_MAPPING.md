...
This document maps the implemented controller to the contract in `DESIGN.md`.
The controller never creates, owns, configures, or lifecycle-manages
CloudNativePG `Cluster` objects beyond its narrow username patch.
...
| Explicitly managed restart/failover signals, each `WATCH_NAMESPACE` | Not registered because the contract names no concrete signal GVK | Restart, failover, and switchover episodes are observed through Cluster and Pod topology. |
...
`vault-replica-controller-state`; the Event grant is only for the
controller-runtime leader-acquisition record and grants no Event read or
watch. No permission to read/watch the CNPG controller Deployment is required.
...
| Value | Source/contract | Implementation |
| --- | --- | --- |
...
| Target credential Secret name/key | Read from each Cluster's selected `externalClusters[].password.name` and `.key` reference; `password` is the expected key for this design | Derived from the current Cluster and blindly merge-patched through a direct REST request; never read or watched. |
| Vault address and authentication settings | TLS-validated Vault endpoint and unattended auth method are prerequisites; E2E may explicitly allow plain HTTP for the ephemeral dev Vault | `VAULT_ADDR` and `VAULT_TOKEN` are required; HTTP requires the explicit E2E opt-in. Requests have configurable timeouts and bounded retry/backoff. |
| Vault database role/path | Derive the role from the selected source CNPG Cluster name, addressed as `/database/creds/<source-cluster-name>`; the source external-cluster entry uses that same stable name | The HTTP client validates the source as a DNS label and issues only from the derived path. |
| Minimum Vault lease duration | Configured lower bound for returned `lease_duration` | Enforced against the authoritative returned duration; default `768h`. |
| Lease safety margin | Configurable margin before `expiresAt` | Enforced before every workflow continuation; default `24h`. |
| Password propagation delay | A few seconds; example is five seconds; persisted as `nextActionAt` | Five-second default implemented as a persisted timed requeue. |
| Verification delay | Typically two to five minutes after username patch | Two-minute default implemented as a persisted timed requeue. |
| Stage deadlines | Suggested: one minute for issue/persist and password-to-username transition; five minutes for reconnect verification | Persisted one-minute and five-minute defaults drive retry/replacement decisions. |
| Orphan sweep interval | Example five minutes | Leader-only five-minute default using fresh API reads. |
| Required consecutive absences | Two confirmed absences before cleanup | Two process-local confirmations are required; restart fails safe by resetting them. |
| State size ceiling | 256 KiB decoded JSON application ceiling | Strict decode and every encode reject state above 256 KiB by default. |
...
The manager wires these settings into the Vault client, durable reconciler,
metrics collector, scoped caches, and leader-only orphan sweeper.

## State fields reserved by the design

The state model contains only the recovery information that cannot be
...
- Creating, updating, deleting, owning, or lifecycle-managing CNPG `Cluster`
  resources beyond the narrowly scoped username patch described above.
- Creating or reading target credential Secret data. The controller may
